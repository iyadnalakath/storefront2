...
# to avoid product id error also update in views.py

    def create(self, validated_data):
        product_id=self.context['product_id']
        return Review.objects.create(product_id=product_id,**validated_data)

class CartPoductItemSerializer(serializers.ModelSerializer):
     class Meta:
         model=Product
...
class CartItemSerializer(serializers.ModelSerializer):
    product=CartPoductItemSerializer(read_only=True)
...
class CartSerializer(serializers.ModelSerializer):
    id=serializers.UUIDField(read_only=True) # for seeing uuid intead of seeing id (ready only=true for avoiding asking id in post field)
...
class OrderItemSerializer(serializers.ModelSerializer):
    product=CartPoductItemSerializer()
    class Meta:
        model=OrderItem
